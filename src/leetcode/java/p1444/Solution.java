    ...
    private void init(String[] pizza, int k) {
        ...
        for (int r = m - 1; r >= 0; --r) {
            for (int c = n - 1; c >= 0; --c) {
                ...
                if (c < n - 1)
                    numOfApples[r][c][0] += numOfApples[r][c + 1][0];
            }
        }
        // columns
        for (int c = n - 1; c >= 0; --c) {
            for (int r = m - 1; r >= 0; --r) {
                ...
                if (r < m - 1)
                    numOfApples[r][c][1] += numOfApples[r + 1][c][1];
            }
        }
        // remaining piece
        ...
        for (int r = m - 1; r >= 0; --r) {
            for (int c = n - 1; c >= 0; --c) {
                ...
                if (c < n - 1)
                    applesInRemainingPiece[r][c] += applesInRemainingPiece[r][c + 1];
            }
        }
        for (int c = n - 1; c >= 0; --c) {
            for (int r = m - 2; r >= 0; --r) {
