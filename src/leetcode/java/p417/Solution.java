...
public class Solution {
    ...
    private void dfs(final int[][] heights, boolean[][] visited, final int r, final int c) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nr < 0 || nc < 0 || nr >= m || nc >= n)
                continue;
            if (visited[nr][nc] || heights[r][c] > heights[nr][nc])
                continue;
            visited[nr][nc] = true;
    ...
    private List<int[]> beachCoordinates(final Ocean ocean) {
        ...
        if (Ocean.PACIFIC.equals(ocean)) {
            for (int r = 0; r < m; ++r)
                coordinates.add(new int[] { r, 0 });
            for (int c = 1; c < n; ++c)
                coordinates.add(new int[] { 0, c });
        }
        if (Ocean.ATLANTIC.equals(ocean)) {
            for (int r = 0; r < m - 1; ++r)
                coordinates.add(new int[] { r, n - 1 });
            for (int c = 0; c < n; ++c)
                coordinates.add(new int[] { m - 1, c });
        }
        return coordinates;
    }
}