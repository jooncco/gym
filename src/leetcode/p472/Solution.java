...
public class Solution {
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for (String word : words) {
            if (dfs(root, word, 0, 1)) ans.add(word);
        }
        return ans;
    }
    ...
    private boolean dfs(TrieNode curNode, String word, int curIdx, int curCnt) {
        ...
        if (curNode.isEndOfWord()) {
            if (dfs(root.next(word.charAt(curIdx)), word, curIdx+1, curCnt+1)) ret= true;
        }
        if (dfs(curNode.next(word.charAt(curIdx)), word, curIdx+1, curCnt)) ret= true;
        return ret;
    }
}
