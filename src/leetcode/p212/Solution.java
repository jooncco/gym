...
public class Solution {
    ...
    private void dfs(char[][] board, TrieNode node, int y, int x, List<String> ans) {
        if (board[y][x] == '.' || node.next[board[y][x]-'a'] == null) return;
        ...
        board[y][x]= '.';
        if (y > 0) dfs(board, node.next[idx], y-1, x, ans);
        if (x > 0) dfs(board, node.next[idx], y, x-1, ans);
        if (y < m-1) dfs(board, node.next[idx], y+1, x, ans);
        if (x < n-1) dfs(board, node.next[idx], y, x+1, ans);
    ...
    private TrieNode constructTrie(String[] words) {
        ...
        for (String word : words) {
            ...
            for (char c : word.toCharArray()) {
                int idx= c-'a';
                if (parent.next[idx] == null) parent.next[idx]= new TrieNode();
