...
public class Solution {
    ...
    private int howMany(int idx, int m, int tight, int odd, int even, int zero) {
        if (idx == n) return odd+even > 0 && odd == even && m == 0 ? 1 : 0;
        if (cc[idx][m][tight][odd][even][zero] > -1) return cc[idx][m][tight][odd][even][zero];
        ...
        if (tight == 1) {
            for (int i=0; i <= upperBound.get(idx); ++i) {
                ...
                cc[idx][m][tight][odd][even][zero] += howMany(idx+1, nextM, i == upperBound.get(idx) ? 1 : 0, nextOdd, nextEven, nextZero);
            }
        } else {
            for (int i=0; i < 10; ++i) {
                nextM= (10*m + i)%k;
