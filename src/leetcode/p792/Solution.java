...
public class Solution {
    public int numMatchingSubseq(String s, String[] words) {
        ...
        for (int i=0; i < s.length(); ++i) ++vector[s.charAt(i)-'a'];
        ...
        for (String word : wordsMap.keySet()) {
            if (isSubsequence(s, vector, word)) ans += wordsMap.get(word);
        }
        return ans;
    }
    ...
    private boolean isSubsequence(final String s, final int[] vector, final String word) {
        ...
        for (int i=0; i < word.length(); ++i) ++wordVector[word.charAt(i)-'a'];
        for (int i=0; i < 26; ++i) {
            if (vector[i] < wordVector[i]) return false;
        }
